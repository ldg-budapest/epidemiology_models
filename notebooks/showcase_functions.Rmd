...
standardized_table <- example_dataset %>%
  calculate_standardized_rate(
    ...
    standard_population = .create_weight_table_from_population(
      filter(example_dataset, Age %in% seq(50, 59)), "2011"
    )
  )%>%
  mutate(
    Age = "50-59"
...
standardized_table <- example_dataset %>%
  calculate_standardized_rate(
    extra_grouping_cols="Diagnosis",
    standard_population = .create_weight_table_from_population(
      filter(example_dataset, Age %in% seq(60, 69)), "2011"
    )
  )%>%
  mutate(
    Age = "60-69"
  ) %>%
  bind_rows(standardized_table)

standardized_table <- example_dataset %>%
  calculate_standardized_rate(
    extra_grouping_cols="Diagnosis",
    standard_population = .create_weight_table_from_population(
      filter(example_dataset, Age %in% seq(70, 79)), "2011"
    )
  )%>%
  mutate(
    Age = "70-79"
